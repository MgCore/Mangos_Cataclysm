...
template<class T>
inline void LoadDB2(uint32& availableDb2Locales, barGoLink& bar, StoreProblemList1& errlist, DB2Storage<T>& storage, const std::string& db2_path, const std::string& filename)
{
...
void LoadDB2Stores(const std::string& dataPath)
{
    ...
    {
        sLog.outError("\nIncorrect DataDir value in mangosd.conf or ALL required *.db2 files (%d) not found by path: %sdb2", DB2FilesCount, dataPath.c_str());
        exit(1);
    }
    ...
    sLog.outString(">> Initialized %d db2 stores.", DB2FilesCount);
    sLog.outString();
}
